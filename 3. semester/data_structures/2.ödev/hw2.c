...
int main(int argc, char *argv[]){
	...
	for(count = 0; count < computernum; count++){
		int s,q,t,f,r;
		computer *co = computers + count;
		s = co->interrupt.maxSize;
		q = co->process.maxSize;
		t = co->interrupt.top;
		f = co->process.front;
		r = co->process.rear; 
		printf("stack size: %d\ntop: %d\nqueue size: %d\nfront,rear: %d,%d\n\n", s,t,q,f,r);
	...
	for(count = 0; count < commentnum; count++){
		if(fscanf(input2,"%c %d %c\n", &one, &two, &three) != 3){
			...
			printf("\n\nerror, one is neither of the commands given\none: %c\n\n", one);
	}
	...
	for(count = 0; count < computernum; count++){
		...
		for(count2 = 0; count2 < computers[count].history.top; count2++){
			printf("%c", computers[count].history.element[count2]); printf(" ");
...
char front(queue q){
	...
	return q.element[(q.front + 1) % q.maxSize];
