...
class Layer:
		...
		def __init__(self, weightNum, neuronNum, func):
			self.bias = np.random.randn(neuronNum)
			self.func = func
			self.inpV = None
			self.weights = np.random.randn(neuronNum, weightNum)
			return	
...
class NeuralNetwork:
	...
	# returns the weight list starting from first layer.
	def weights(self):
		ret = []
	...
	def miniBatchUpdate(self, minibatch, rate): 
		wder = []
		bder = []
		for layer in self.layers:
			wder.append(np.zeros(layer.weights.shape)) ## each neuron has its own lamda value
		...
		for inp in minibatch:
			feature = inp[0]
			y = inp[1]
			lastAct= self.feedForward(feature)
		#	print(lastAct); print("*****")
	...
	# trains the network with given parameters. Returns the weight vector that found in last epoch		
	def train(self, inp, miniBatchSize, rate, epoch):
		global error
		for e in range(epoch):
			...
			for batch in miniBatches:
				self.miniBatchUpdate(batch, rate)
			error = error / len(inp)
			print("epoch is " + str(e) + "\terror is " + str(error))
			
	# predicts the class
	def predict(self,vector):
		ret = self.feedForward(vector)
		return np.argmax(ret)

	def accuracy(self,inpList):
		length = len(inpList)
		trueGuess = 0
		for el in inpList:
			if(self.predict(el[0]) == 1):
				trueGuess += 1
		return trueGuess / length

# returns a network with given weights.
def initWithWeights(self, weights, functionV):
	neuronNumV = [len(layer) for layer in weights]
	classNum = len(weights[-1])
	featureNum = len(weights[0][0])
	ret = NeuralNetwork(neuronNumV, functionV, classNum, featureNum)
	for i in range(len(ret.layers())):
		layer = ret.layers[i]
		layer.setWeights(weights[i])
	return ret

#scales vector. It is neccesary for not getting zero, nan or inf caused by floating point overflow-underflow
def scale(vector):
	return np.divide(vector, 256)
