...
public class UserCollection {
	...
	public static Collection<TextPost> allPosts = new ArrayList<TextPost>();
	
	/**adds user to the system.
	 * does not add if given userName is already exists
	 * @return true if user added. else false*/
	public static boolean addUser(String name, String userName, String password, Date dateOfBirth, String school,String relationship) {
		...
		User user = new User(name, userName, password, dateOfBirth, school,relationship);
		users.add(user);
		return true;
	...
	public static boolean removeUser(String userName) {
		User user = User.sUserNametoUser(userName);
			...
			for(TextPost post : user.getPosts()){
				allPosts.remove(post);
			}
			for(TextPost post : allPosts){
				post.removeTag(user);
			}
			users.remove(user);
